...
class RunManager:
    def __init__(self, date_range: List):
        ...
        self.api = wandb.Api(timeout=120)
    
    def fetch_runs(self):
        self.__get_projects()
        self.__get_runs()
        self.__get_metrics()
        combined_df = self.__create_combined_run_df()
        return combined_df
    ...
    def __get_runs(self):
        for team_config in self.team_configs:
            if team_config.team == "stockmark-geniac":#stockmark
                ...
                for i, project in enumerate(team_config.projects):
                    ...
                    # print(f"Retrieved {len(project.runs)} runs for {project.project}")
            else:
                print(f"Skipping runs for {team_config.team}")
                team_config.projects = []
    
    def __get_metrics(self):
        print("Get metrics for each run ...")
        for team_config in self.team_configs:
            if team_config.team == "stockmark-geniac":
    ...
    def __create_combined_run_df(self):
        ...
        if combined_df:
            total_runs = sum(len(df) for df in combined_df)
            print(f"Total runs processed: {total_runs}")
            return pl.concat(combined_df)
        else:
    ...
    #         except Exception as e:
    #             print(f"Error executing GraphQL query for {team}/{project}: {str(e)}")
    #             break
    ...
    #         if self.__is_run_valid(node, createdAt, updatedAt):
        ...
        while True:
            for attempt in range(max_retries):
                try:
                    ...
                    _edges = results["project"]["runs"]["edges"]
                    if not _edges:
                        print(f"Finished querying all runs for {team}/{project}. Total processed: {total_processed}")
                    ...
                    if attempt < max_retries - 1:
                        print(f"Error querying {team}/{project}. Retrying in {retry_delay} seconds... (Attempt {attempt + 1}/{max_retries})")
                        print(f"Error details: {str(e)}")
                        time.sleep(retry_delay)
                    else:
                        print(f"Failed to execute query for {team}/{project} after {max_retries} attempts.")
                        print(f"Error details: {str(e)}")
                        return self.__process_nodes(nodes, team, project)
    ...
    def __is_run_valid(self, node, createdAt, updatedAt) -> bool:
        ...
        # 特定のtagをスキップ
        if not set(CONFIG.ignore_tag).isdisjoint([t.lower() for t in node.tags]):
            return False
    ...
    def __process_project_runs(self, project):
        max_workers = min(32, len(project.runs))  # ワーカー数を制限
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self.__create_metrics_df_with_retry, run.run_path): run for run in project.runs}
            completed = 0
            for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc=f"Processing {project.project}"):
                run = futures[future]
                try:
        ...
        print(f"Completed processing {completed}/{len(project.runs)} runs for {project.project}")

    @timeout_decorator.timeout(60)  # 1分のタイムアウト
    def __create_metrics_df(self, run_path: str) -> pl.DataFrame:
        ...
        try:
            ...
            raise  # 例外を再発生させ、__create_metrics_df_with_retryでキャッチできるようにする
    
    def __create_metrics_df_with_retry(self, run_path: str, max_retries=3, initial_timeout=120):
        for attempt in range(max_retries):
            try:
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(self.__create_metrics_df, run_path)
                    return future.result(timeout=initial_timeout * (attempt + 1))
    ...
    def __create_run_df(self, run: Run) -> pl.DataFrame:
        ...
        return new_run_df.with_columns([
            ...
            "max_gpu_utilization", "max_gpu_memory", "logged_at"
        ])
...
if __name__ == '__main__':
    ...
    combined_df = run_manager.fetch_runs()
    csv_filename = "combined_runs_data.csv"
